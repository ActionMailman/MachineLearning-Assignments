...
class Grid {
  ...
  draw() {
    for (let row = 0; row < this.rows; row++)
      for (let col = 0; col < this.cols; col++) this.drawSquare(col,row,this.gridColor);
  }
  ...
  drawCurrency(currency) {
    // Change this print to a text() and display the agent position at the top of the canvas
    print(currency.col + " " + currency.row);
    this.drawSquare(currency.col,currency.row,color(0,255,0));
  }
}
